...
| Dev-50 | Paper-over-paper parallax at every torn section boundary on every route (home Featured Work over the hero copy, every later home/tab section, case-study sections, the band footer): the section above drifts at 0.5× page speed while the torn sheet below keeps 1×, overlapping its lower ~200 px (phones < 768: 0.5× to 120 px); a slow drift, not a pinned/sticky stack; zero under reduced motion | Tushar 2026-09-26: "for all paper cutout create a parallax effect in scrolling" / "the 'Feature Worked' should scroll above the hero section, it should be the case for all paper cut segment in all tabs". Generic CSS keyed on the TornEdge first-child contract (§3.1) — CSS scroll-driven `view()` timeline starting once the tear is 80 px into the viewport (`entry-crossing`, any section height), `translate` only; `main` never moves; keyboard focus inside a lagging section drops the lag (WCAG 2.4.11) (no layout shift, no JS); the scene openers keep their Dev-39 image parallax (TKT-106) | Tushar (2026-09-26) |
