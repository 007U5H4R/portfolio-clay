...
| `/` hero | hand-sub annotation · h1 underline sketch · scene caption annotation | 3 → **actual (TKT-108, Dev-50)**: banner section 1 (postmark); nested `section#hero-copy` 4 — torn · h1 underline sketch (+ CSS accent strokes) · hand-line annotation · marginalia `collage` | poster/clip illustration; Tushky sticker + its pointer (decorative illustration, `aria-hidden`) |
...
- **Hero copy block** (TKT-108, Tushar direction 2026-09-26, Dev-50 — supersedes the §5.1 copy lines; the banner above is unchanged): rebuilt to `docs/redesign-mockups/m-009/tushar-2026-09-26/hero-copy-target.png`, every string from `data/hero.ts` (Tushar's own copy, `VERIFIED` by authorship): eyebrow "SENIOR PRODUCT MANAGER · ENTERPRISE AI · AI-NATIVE BUILDER" (Inter 14 px, 0.26em tracking); h1 "I turn messy problems into / AI-native products / people actually use." (Fraunces `clamp(34px, 5.6vw, 80px)`, the three reference lines as blocks ≥ 768, natural wrap below) with the rust underline under "people actually use." and two rust accent strokes up to its right (CSS on the same host — one sketch object); Caveat hand line "Same curiosity. Bigger problems. Better products." (`aria-hidden` annotation, faint stroke under "problems."); support "7+ years across product, cloud, data and AI — from enterprise platforms at Godrej, Quantiphi, Shellkode and American Express to independently built AI products tested with real users." (Inter 17–20 px, ≥ 768); CTAs: rust Fraunces pill "View my work →" → `/projects`, ivory Fraunces pill "✦ Ask Tushky" → `#ask` (unchanged behaviour; `aria-describedby` the 13 px `data-micro-label` caption "My AI portfolio assistant"), with Tushky's head sticker (the banner's golden retriever, 64 px, `alt=""`, `aria-hidden`, lazy) and a small pointer arrow ≥ 640. Marginalia + faint backdrop as ONE `collage` object: Caveat notes "real problems / real people / bigger impact" (with an arrow to the h1) and "problem → insight → build → learn" (faint underline) ≥ 1024 only; pale torn patches, a leaf shadow bottom-left, a grid-paper corner bottom-right and a hairline loop top-right ≥ 768. 390: annotations hidden, CTAs side by side / wrap, no overflow.
...
| Dev-50 | Home hero copy block rebuilt to Tushar's reference 2026-09-26 (TKT-108): new eyebrow / h1 / hand line / support copy, Fraunces CTA pills "View my work →" (→ `/projects`) and "✦ Ask Tushky" (+ caption, head sticker), Caveat marginalia and a faint backdrop (§7.1). Decoration accounting: (a) the hero's paper sheet becomes its own nested `<section id="hero-copy">` — a §3.2 rule-1 counting unit, as nested chapter sections already are — so the banner section counts 1 (postmark) and the sheet 4 (torn · underline · hand line · collage) at every width; (b) the `collage` kind (Dev-41) is extended to carry `aria-hidden` Caveat marginalia (the two margin notes) alongside its text-free pieces, still counted once; (c) the h1's two accent strokes and the hand line's faint stroke are CSS on their hosts (part of the same sketch / annotation objects, like Dev-41's CSS tape strip); (d) the Tushky sticker and its small pointer are one decorative illustration (`alt=""`, `aria-hidden`), not a `data-decor` object — like the polaroid photos. The primary CTA's label moves from Caveat (`data-hand="cta"`) to Fraunces, per the reference | Tushar: "change the hero section to this" (`docs/redesign-mockups/m-009/tushar-2026-09-26/hero-copy-target.png`); without (a)–(d) the reference's ornaments would be 8 objects in one unit against the ≤ 4 budget (S15), which is not re-opened. The sticker art is job `36837247-25de-4830-8f58-c069b00d2188` (provenance in content/media/illustrations/README.md) | Tushar (2026-09-26) — accounting (a)–(d) is the implementer's default, **confirm at review** |
