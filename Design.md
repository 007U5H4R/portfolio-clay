...
2. **What counts.** Every element with a `data-decor` attribute (`torn | sticky | annotation | sketch | note | tape`) — one each, regardless of size. `data-fastener` and `data-paper` never count (D6 narrows EV5's "tape" to *free-standing* tape).
...
| `/` how I think | torn · journey path sketch | 2 | 6 pinned stage cards; stage quotes `data-hand="quote"` |
...
| `/about` product journey | torn · path sketch · "start here ↘" annotation | 3 | 4 pinned year cards; closing line is Fraunces lead + DraftTag |
...
| Dev-23 | New manifest entry `hero-banner` (outpaint of `hero-desk`, 3168×1344) with alt "Illustration of Tushar at a warm desk — laptop, notebook, books, plants, a lamp, a sleeping golden retriever, and pinned notes reading Problem → Insight → Bet → Build → Evaluate → Impact."; polaroid crops are decorative (`alt=""`, `aria-hidden`) because each scene is described on its own page | §6.3 alts must match what is shown; polaroids repeat pictures, not information | Stage-7 default (orchestrator) — confirm at the gate |
