...
| Dev-70 | Home "How I think" plays once as a paper-unrolling choreography (~7.6 s) instead of the §8 "Section reveal" 70 ms stagger: the collage backdrop is revealed centre-out (`clip-path: circle(0% → 100% at 50% 50%)`, 1.4 s), then each banner unrolls top → bottom (clip-path, 750 ms incl. a ~300 ms recoil) strictly after the previous one settles, its pin lands at ~85 %, its content fades in after ~30–55 %; triggered at 30 % visibility, never replays, never locks scroll; narrow widths unroll each card as it is reached (still in order); reduced motion / no-JS / already-on-screen = final state at once, focus into the board skips to the end | Tushar's choreography spec (`docs/redesign-mockups/m-009/tushar-2026-09-26/how-i-think-choreography-spec.md`) — §8's generic fade-up is exactly what it rejects | Tushar direction 2026-09-26 (TKT-110) |
| Dev-71 | The How-I-think journey line is drawn ABOVE the cards (z 4 over cards z 2), measured pin to pin (lead-in, five joins, a short tail) and arcing through the top margin so it never crosses text; each segment is drawn in step with its card (dash pattern kept via a per-segment mask). Pins stay visible on top because every segment stops at the pin's rim (the pin lives inside the card's own stacking context) | spec §2 / §3 / §20 | Tushar direction 2026-09-26 (TKT-110) |
| Dev-72 | How-I-think card rotations become Problem −0.5° · Insight +0.35° · Bet −0.25° · Build +0.4° · Evaluate −0.35° · Impact +0.3° (was alternating −0.8° / +0.6°) | spec §11 paper physics | Tushar direction 2026-09-26 (TKT-110) |
