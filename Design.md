...
| Dev-60 | Ask panel becomes the **Ask Tushky right drawer with a multi-turn chat** (supersedes Dev-47's torn notebook and the S21 drawer-vs-bottom-sheet geometry). It is fixed top/right, 100dvh, `clamp(400px, 32vw, 460px)` wide, with a torn LEFT edge and a soft left shadow. Below 768 it is 100vw full-screen instead of a bottom sheet. The backdrop is navy 38% with a 2 px blur. Opening is a 420 ms `cubic-bezier(.22,1,.36,1)` slide from the right with a 4 px paper settle; closing is a 280 ms slide back with no bounce. It has a compact header (Fraunces "Ask Tushky" + a paw, a Caveat subtitle with a hand underline, a 44 px round close) and one scrolling body with the composer pinned. The empty state is the mascot, one grounding sticky note, the intro and six suggestions. After the first question the body becomes a real conversation: user bubbles on the right; Tushky bubbles on the left with a 32 px avatar, "Sources from portfolio:" chips (the evidence titles the index returns) and 2–3 follow-up chips on the latest answer. The grounding note replaces the S7 microcopy line inside the drawer and is not repeated in the chat | Tushar's spec 2026-09-26 (`docs/redesign-mockups/m-009/tushar-2026-09-26/ask-tushky-drawer-spec.md`). Behaviour kept: retrieval-only local index (EVAL-012 unchanged), verbatim answers, native `<dialog>` + `showModal()` (the top layer gives spec §4's page < backdrop < drawer stack), focus trap / Esc / backdrop close / focus return to the opening trigger, lazy chunk (EVAL-005), `lockBackground`. The user's own words now render in their bubble as a React text node, never as HTML. Motion is CSS, not `motion/react`, because the native dialog needs imperative close sequencing; reduced motion appears in place | Tushar direction 2026-09-26 (TKT-104 r2) |
| Dev-61 | The composer has **no attachment (paperclip) button**, although the spec draws one | Tushky answers only from the portfolio index and cannot read files. A control that does nothing, or that silently drops a file, would make a false promise. The slot renders nothing: input + send only | Orchestrator constraint 2026-09-26 (TKT-104 r2) — confirm at the gate |
| Dev-62 | Manifest entry `tushky` v2 (kind `mascot`, public-only `/media/illustrations/tushky-bandana.webp`, 231×280; supersedes Dev-48's `tushky.webp`, which is removed) with alt "Tushky, the golden retriever portfolio assistant, wearing a navy bandana lettered Tushky"; new manifest entry `tushky-avatar` (kind `mascot`, public-only `/media/illustrations/tushky-avatar.webp`, 64×64, rendered decoratively with alt="") with alt "Tushky, the golden retriever portfolio assistant (chat avatar)" | Spec §9: a golden retriever in a navy bandana that says "Tushky", no badge. Higgsfield `gpt_image_2_5`, job `20288256-cee5-4dcb-aa02-73a6e8be3647`, 0.5 cr, reference = the round-1 bandana alternate `7312a2c0-…`; accepted on the first attempt with the name spelled correctly. Both files load only inside the lazy drawer chunk. Provenance rows are in `content/media/illustrations/README.md` (EVAL-021) | Tushar approved the spend 2026-09-26 (TKT-104 r2) |
| Dev-63 | The drawer's suggestions are Tushar's six questions in a single column (data-driven `{label, query, category}`, pastel `color-mix()` category tints), replacing the six `surface:'panel'` prompts and the Dev-49 quick-action chips (retired, including "Browse my projects"). Two cards submit a remapped query under Tushar's wording: "Show me his product thinking process." → the `discovery` prompt, and "Walk me through a specific project." → the `most-technical` prompt (RailCite). Follow-up chips are derived from the answer's sources (other entries sharing evidence pages) and submit exact entry prompts | Grounding: as worded, both remapped questions matched the product list (`built`). "What is his AI / cloud experience?" resolves to `ai-products` (0.67); the index has no cloud-specific entry. The spec's example chip "Compare his experience" returns the empty fallback, so it is never offered. `tests/unit/ask-tushky-grounding.test.ts` pins every card and follow-up to a sourced answer | Tushar direction 2026-09-26 (TKT-104 r2) |
