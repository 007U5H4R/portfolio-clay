...
| `/work` work experience (TKT-101, Dev-43) | hand-lettered title strip `note` · aside annotation "Different problems. Same curiosity. Bigger impact." · collage backdrop (vignettes, doodles, platform marks, stickies — one object, TKT-99 Dev-40 kind) | 3 | 4 deckled cards (`data-paper="card"`), 4 taped logo cards (`data-paper="photo"`); the real h2 is sr-only |
| `/work` education (TKT-101, Dev-43) | torn (the paper cut-out) · title strip `note` · aside annotation "From engineering foundations to research-driven thinking." · collage backdrop | 4 | 2 deckled cards, 2 taped logo cards; sr-only h2 |
| `/projects` opener (was `/work`, TKT-101) | h1 underline sketch · scene caption annotation | 2 | scene bleed illustration |
| `/projects` index (was `/work`) | torn · "start here ↓" annotation (arrow hidden < 900 stays in the object) · RailCite sticky "trust is the product." | 3 | flagship opener card (taped), rows; `EmptyState` index card renders only when empty (Dev-05) |
| `/projects` experience strip (was `/work`) | torn | 1 | `↳` is CSS list chrome |
...
| `/certifications` timeline (`section#certifications`, TKT-102) | hand-lettered title strip (`note`) · aside annotation "Credentials are useful. Applied capability is better." · collage (Dev-40 kind: generated paper scraps, leaf, fern, mountain, stars, sparks — hidden < 1280/1100 but kept in the DOM) · footer note (books LEARN/BUILD/APPLY/GROW, paper plane, "Continuous learning for higher impact.") | 4 | 5 credential links, each holding a year tape, a round deckled badge (credential artwork, Dev-47), a torn card (`h2` name) and a pinned sticky note (the card's own annotation, `aria-hidden`, reached via `aria-describedby` — Dev-46) |
| `/certifications` more on Credly (`section#more-credentials`) | torn | 1 | 17 credential tiles (content paper) |
...
### 7.2 `/work` — Experience: work experience → torn cut-out → education → band (TKT-101); `/projects` — opener → index → experience strip → band
- **`/work` (TKT-101, Tushar direction 2026-09-26; Dev-42…45).** References `docs/redesign-mockups/m-009/tushar-2026-09-26/work-experience-target.png` + `education-target.png`. No scene opener (the reference starts at the title; the pinboard scene moved with the index to `/projects`). sr-only h1 "Experience". Two `CollageTimeline` sections (`components/experience/`): **Work Experience** (`section#work-experience`, `paper`) and **Education** (`section#education`, `paper-2`, torn top = the paper cut-out). Head: the hand-lettered title on a torn note-yellow strip (a `note` decoration; the real `<h2>` is sr-only with the same words) + the Caveat aside with a down arrow. Timeline = `<ol>`, newest first, grid `250px · 56px rail · 1fr card · 196px side`: a tinted date blob (Fraunces italic, `<time>`; a hand arrow toward the rail) → a taped logo card (`data-paper="photo"`, the real logo — Dev-44 — or the name in Fraunces when no official file exists) overlapping the lower-left of a generated building/campus vignette → the rail pin (tone colour) → a deckled torn-paper card (two seeded clip-path layers, as TKT-99/100): h3 organisation (Fraunces 650), city chip (Dev-45), role (Fraunces italic over a tone stroke), bullets with tone dots, and (Work) the résumé outcomes as chips under the micro-label "Outcomes · self-reported". Right side: the collage's doodles (GenAI cloud, database, gears, flow, API bubble, bulb, atom, microscope, network), the AWS / Google Cloud marks and the reference's stickies — all inside ONE aria-hidden collage object that shares the list's rows through `subgrid`. Copy = `data/experience.ts` (context · responsibility · scale when recorded · what changed; outcomes) and `data/credentials.ts` (degree, institution split at its last comma into name + city, year; `highlights` when supplied). ≤ 1024: no side column, the wide collage pieces hidden; < 768: one column, date + logo side by side then the card, no rail, no vignette. **Motion:** while Education enters the viewport the Work content drifts down at half speed (`xp-drift`, a view timeline on `#education` via `timeline-scope`), so the Education sheet slides up over it with the torn edge leading — the TKT-96 mechanism (CSS scroll-driven, `@supports`, off under reduced motion). **States:** static SSR data (working only; empty/loading/error n/a).
- **`/projects`** (the former `/work` index, moved unchanged by TKT-101; h1 "Work" → "Projects"; case studies keep `/work/<slug>`, their crumb and the 404's "See the work" point at `/projects`):
...
| Dev-46 | New `/certifications` tab + nav item "Certifications" — a collage timeline of the five certifications in Tushar's reference (newest first by Credly issue date) + a "More on Credly" shelf of the other 17 badges; each certification is ONE `<a>` to its individual Credly credential (new tab), whole card clickable, Caveat "View credential ↗" micro-label, hover lift −4 px / −0.6° / deeper shadow + ↗ nudge (reduced motion: shadow only). No scene opener (no page scene exists; no new generated imagery) — opens on the torn title strip. Sticky notes are the card's own annotation: `aria-hidden` (Caveat), their words in the link's `aria-describedby`, not counted as decorations; section count 4 (title note · aside · collage · footer note). Dates / names stay Fraunces (§3.4), not the reference's marker lettering. PMP / SAFe are named on these surfaces only — now Credly-verified (CONTENT_INVENTORY §4.6's "banner-only" exclusion stands everywhere else; `CREDENTIAL_SURFACES` in scripts/forbidden-strings.ts)  Header fit: at 1024–1279 the nav drops to Fraunces 15 with a 16 px gap and the pill to 17 px / 12 px side padding, and at 1280–1439 the nav gap to 22 px, so six items pass QA-010 (≥ 16 px between controls); seven items (with TKT-101's Projects) do **not** fit at 1024–1279 even so — see docs/reports/TKT-102.md | Tushar direction 2026-09-26 (TKT-102): certifications-target.png + certifications-spec.md | Tushar (2026-09-26) |
| Dev-56 | Credential badge artwork (Google Cloud, PMI, Scaled Agile, Scrum.org, AWS badges from Credly) renders as real images on `/certifications` — an EVAL-021 logo exception: they are Tushar's credentials, not brand decoration; files + sources in `public/media/certifications/README.md` | Tushar approved real logos 2026-09-26 | Tushar (2026-09-26) |
| Dev-80 | Polaroids pinned onto the banner's blank papers — Tushar direction 2026-09-26 (TKT-111) | Tushar: "move polaroids to blank canvases behind". TKT-105 left the wall papers blank, so the three polaroids (`scene-about`, `scene-playground`, `scene-work`) each sit taped inside one blank paper — the tall cream sheet left of the character, the square yellow note top-right, the large cream sheet right — tilted ≤ 2°, positioned and sized in banner-canvas pixels so they track the scene at every width ≥ 768; hidden < 768 (4:3 crop). Three papers, three polaroids, so no new polaroid was generated; the mountain photo and the now-clean corkboard stay bare. Replaces Dev-21's "polaroids over the corkboard / bottom-left" placement and the TKT-98 corkboard guard | Tushar (2026-09-26) |
| Dev-90 | `/work` becomes **Experience** and the project index moves to a new **`/projects`** tab; the nav grows from five items (D8) to six: Home · Experience · Projects · Thinking · About · Playground — Tushar direction 2026-09-26 (TKT-101) | Tushar: "in the work tab, remove everything whatever is there" / "change the tab name "Work" to "Experience"", and the 11-project index goes to "a new /projects nav tab". Case studies keep `/work/<slug>` (no broken links or OG). `?filter=` hrefs, the home "View my work →" CTA, the case-study crumb, the 404's "See the work", the Ask evidence links, the sitemap and the EVAL-017 route list now point at `/projects`. The `scene-work` opener moved with the index, so `/work` has no scene opener. QA-010 (header fit at 1024 / 1280 / 1440) is re-run with six items: they did not fit (77 px over at 1024, a 12 px pill overlap at 1280), so 1024–1279 gets 32 px header gutters, 16 px nav type and a 16 px nav gap, and 1280–1439 a 24 px nav gap. Items, order and the 44 px hit areas are unchanged, and ≥ 1440 is untouched | Tushar direction 2026-09-26 (TKT-101) |
| Dev-91 | `/work` Work Experience + Education as collage timelines: hand-lettered title strips (Caveat as a `note` decoration, sr-only `<h2>`), one collage object per section, Fraunces italic dates/roles in place of the reference's handwriting, Education sliding over Work on scroll — Tushar direction 2026-09-26 (TKT-101) | References `work-experience-target.png` / `education-target.png`. Counting every doodle, sticky and vignette separately would give about 10 decorations per section against §3.2's limit of 4. Instead they sit in one aria-hidden collage (the TKT-99 Dev-40 kind), so Work = 3 and Education = 4 (torn · note · annotation · collage) at both widths, and the parked list stays `[]`. §3.4 bans Caveat on headings and dates, so the title's lettering is decoration with a sr-only heading, and dates, roles and cities use Fraunces italic. The stickies ("From legacy to modern.", "Build Deploy Scale", "Research / Publications / Patent", …) are his reference copy, aria-hidden. The generated building/campus vignettes are decorative (`alt=""`, Higgsfield spend approved by Tushar). The paper cut-out is `TornEdge` on `paper-2` | Tushar direction 2026-09-26 (TKT-101) |
| Dev-92 | Real company / institution logos on `/work` (American Express, Godrej, NIT Calicut; AWS + Google Cloud marks in the collage) — an **EVAL-021 exception** (that eval's "no logos" rule governs the generated illustrations; these are official files, never redrawn) — Tushar direction 2026-09-26 (TKT-101) | Tushar: "use the real company/institution logos"; he owns the trademark-usage call. Files come from Wikimedia and are svgo-optimised. Source URL, licence and trademark note per file are in `content/media/logos/README.md`. NIT Calicut's file is **fair-use on en.wikipedia, not Commons**, flagged to him. Shellkode, Quantiphi and BIT Durg have no official Commons file, so their names are set in type (no invented logo). Logos are content, `alt` = the organisation's name | Tushar direction 2026-09-26 (TKT-101) |
| Dev-93 | Cities shown on the `/work` cards (employer / institution city: Bengaluru, Mumbai, Kozhikode, Durg) — an exception to the `site.showLocation = false` default, which **stays false** for Tushar's own location (band, contact) — Tushar direction 2026-09-26 (TKT-101) | Tushar: "show the cities". The source is CONTENT_INVENTORY §4.5 / §4.8 (RESUME). Work: a new optional `location` on `data/experience.ts`. Education: the city already in `institution`, split at its last comma. An employer's city is not a personal address (no PII) | Tushar direction 2026-09-26 (TKT-101) |
| Dev-94 | Seven-item header nav (Home · Experience · Projects · Thinking · About · Playground · Certifications) shows in full only ≥ 1440; 1024–1439 collapses the links into the MobileMenu button (paper drawer) beside the "Let's connect" pill and the Ask ghost; < 1024 unchanged | Tushar 2026-09-26 chose "Menu below 1440" (seven items do not fit at 1024–1439 by spacing alone — TKT-101/TKT-102 measurements) (TKT-112) | Tushar (2026-09-26) |
