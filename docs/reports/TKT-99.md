...
## Rules
- **EVAL-018 / Design.md §11 Dev-40.** The collage counts as ONE decoration through a new `data-decor="collage"` kind (§3.1 row, §3.2 rule 2). The section count goes from 2 to **3 at 1440** and from 1 to **2 at 390**, still within the ≤ 4 budget. `eval-018-parked.json` is still `[]`. The §3.3 row and the §7.1 note are updated. Two per-card details are recorded in Dev-40:
