...
| M-003 hero rebalance (EXE-9) | TASK-14 | opus | 🔄 next | Shorten hero @390 (primary CTA above fold) + avatar presence @1024 (widen track / trim headline); keep responsive (no F1 overflow); re-run EVAL-001 + regen home screenshots → orchestrator reviews. Then M-003 QA gate → merge → PING. |
