...
describe("primary nav (lib/nav.ts, D8)", () => {
  it("has six items ending in Playground, in the Design.md §4.1 order (TKT-101)", () => {
    expect(navItems).toHaveLength(6);
    expect(navItems.map((item) => item.href)).toEqual(["/", "/work", "/projects", "/thinking", "/about", "/playground"]);
    expect(navItems.map((item) => item.label)).toEqual(["Home", "Experience", "Projects", "Thinking", "About", "Playground"]);
  });
