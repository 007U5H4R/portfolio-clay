/**
 * deckle.ts (TKT-99) — deterministic torn / deckled paper outlines for the home "How I think"
 * section (Tushar direction 2026-09-26, how-i-think-target.png). Pure geometry: returns a CSS
 * `polygon()` in percentages so the same outline scales with its box. Seeded, so the server render
 * is stable (no hydration drift, no per-request variation) and every card gets its own edge.
 */
