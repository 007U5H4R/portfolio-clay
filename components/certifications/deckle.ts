/**
 * deckle.ts (TKT-102) — deterministic torn / deckled paper outlines for `/certifications` (Tushar
 * direction 2026-09-26). A verbatim copy of TKT-99's `components/home/deckle.ts` (TKT-100/101 carry
 * the same copy on their parallel branches) — dedupe into one shared module at merge. Pure geometry:
 * returns a CSS `polygon()` in percentages so the outline scales with its box; seeded, so the server
 * render is stable (no hydration drift).
 */
