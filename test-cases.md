...
- **Type:** visual-review (product acceptance) · **Priority:** P1 · **Automation:** N — manual (anchor existence automated via TC-077) · **Status:** Planned · **Defect:** —
- **Notes:** —
...
| Q8 | What I learned when assumptions failed | TeachSpark drop-off retro + "Bangalore four different ways"; RailCite "staleness is a correctness bug", temperature rejection found by live smoke; Velora "Nothing below presents a hypothesis as a validated fact" + the kill decision | `/work/teachspark#08-what-i-learned`, `/work/railcite#08-what-i-learned`, `/work/velora#08-what-i-learned` | _pending_ | Planned |

Threshold: 8/8 rows with ≥1 verified anchor (evaluation-plan EVAL-003). Rows Q3 and Q4 lean on supplementary non-featured pages; if TKT-39 finds the featured-three artifacts insufficient for either, the fix is a sourced artifact from the pack or an explicit documented gap — never a new claim.
